...
sTaskNode* createNode(sTask task) {
    sTaskNode* newNode = (sTaskNode*)malloc(sizeof(sTaskNode));
    if (newNode == NULL) {
        return NULL;
    }
    newNode->task = task;
...
void deleteNode(sTaskNode** head, uint32_t TaskID) {
    sTaskNode *temp = *head, *prev = NULL;

    // If head node itself holds the TaskID to be deleted
    if (temp != NULL && temp->task.TaskID == TaskID) {
    ...
    // If TaskID was not present in linked list
    if (temp == NULL) return;
...
void freeList(sTaskNode* head) {
    sTaskNode* current = head;
    sTaskNode* next;
